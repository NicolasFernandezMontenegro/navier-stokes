                              ...
                              float* restrict same)
{
    ...
    for (unsigned int y = 1; y <= n; ++y) {
        for (unsigned int x = 0; x < n / 2; ++x) {
            ...
            __builtin_prefetch(&same0[index + 16], 0, 1);
            __builtin_prefetch(&neigh[index + 16], 0, 1);
                      ...
                      float a, float c)
{
    ...
    #pragma omp parallel shared(n, width, a, c, red0, blk, red, blk0, b, x)
    {
                       ...
                       float dt)
{
    ...
    #pragma omp parallel shared(d_Red, d_Blk, u_Red, u_Blk, v_Red, v_Blk) 
    {
